...
import com.r3.developers.samples.tokens.contracts.MetalExchangeContract.Transfer
import com.r3.developers.samples.tokens.states.MetalExchangeState
...
class TransferMetalExchangeTokenFlow : ClientStartableFlow{
    ...
    override fun call(requestBody: ClientRequestBody): String {
        ...
        try{
            val (symbol, issuer, amount, receiver) = requestBody.getRequestBodyAs(
                jsonMarshallingService, TransferMetalExchangeFlowInputArgs::class.java
            )
            ...
            val tokenClaimCriteria = TokenClaimCriteria(
                MetalExchangeState::class.java.getName(),
            ...
            val MetalExchangeStateNew = MetalExchangeState(
                getSecureHash(issuerMember.name.commonName!!),
            ...
            val txBuilder : UtxoTransactionBuilder =
                if (change.compareTo(BigDecimal.ZERO) > 0) {
                    // if there is change to be returned back to the sender, create a new MetalExchange state
                    // representing the original sender and the change.
                    val MetalExchangeStateChange = MetalExchangeState(
                        getSecureHash(issuerMember.name.commonName!!),
                    ...
                    ledgerService.createTransactionBuilder()
                        ...
                        .addOutputStates(MetalExchangeStateChange, MetalExchangeStateNew)
                        .addCommand(Transfer())
                    ...
                    ledgerService.createTransactionBuilder()
                        ...
                        .addInputStates(
                            tokenClaim.claimedTokens.stream().map{ it.stateRef }.collect(Collectors.toList()))
                        .addOutputStates(MetalExchangeStateNew)
                        .addCommand(Transfer())
                        .addSignatories(listOf(myInfo.ledgerKeys[0], receiverMember.ledgerKeys[0]))
                }
            ...
            flowEngine.subFlow(FinalizeMetalExchangeTokenSubFlow(signedTransaction, receiverMember.name))
...
data class TransferMetalExchangeFlowInputArgs(
    val symbol: String,
